...
export const transcribeMlxService = {
  ...
  startMlxTranscription: async (options: TranscribeMlxOptions): Promise<void> => {
    const { tempFilePath, jobId, language, model, storageRepository = defaultStorageRepository, onComplete, onError } = options;
    ...
    try {
      ...
      // 3. 清理暫存檔案
      await transcribeMlxService.cleanupTempFile(tempFilePath, storageRepository);
    }
  },
   ...
   * 清理暫存檔案
   */
  cleanupTempFile: async (tempFilePath: string, storageRepository = defaultStorageRepository): Promise<void> => {
    try {
      await storageRepository.deleteFile(tempFilePath);
